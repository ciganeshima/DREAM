...
def batchify(data, batch_size, is_reordered=False):
    ...
    if is_reordered is True:
        ...
        for i in range(num_batch):
            ...
            baskets, lens, uids, rbks, ihis = sort_batch_of_lists(baskets, lens, uids, rbks,
                                                                  ihis)  # sort, max_len = lens[0]
        ...
        if len(data) % batch_size != 0:
            residual = [i for i in range(num_batch * batch_size, len(data))] + list(
                np.random.choice(len(data), batch_size - len(data) % batch_size))
            print(len(residual))
